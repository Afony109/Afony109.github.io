...
export async function getApyTiersOnChain() {
    ...
    try {
        ...
        // Ethers.js может вернуть или массив [thresholds, apys], или объект
        ...
        // thresholds в контракте в 6 decimals (100_000e6 и т.д.)
        const thresholds = thresholdsRaw.map((v) => {
            const n = v.toString ? Number(v.toString()) : Number(v);
            return n / 1e6; // → обычные доллары, например 100000
        });

        // APY в контракте в basis points (1200 = 12%)
        const apys = apysRaw.map((v) => {
            const n = v.toNumber ? v.toNumber() : Number(v);
            return n / 100; // → проценты, например 12.0
        });
